# Article 113: The Three Lines Model for AI – Defense in Depth

## TL;DR

The Three Lines Model is a proven governance framework that separates risk management into three distinct roles: First Line (business operations that own and manage risk), Second Line (oversight functions that monitor and advise), and Third Line (internal audit that provides independent assurance). Applied to AI, this model ensures defense in depth—multiple layers of governance so that if one fails, others catch problems. This article explains how to apply the Three Lines Model to AI governance, what each line should do, how they work together, and how to avoid common implementation mistakes that leave gaps in AI oversight.

---

## Introduction

A bank deployed an AI model for credit decisions. The data science team (First Line) tested it and said it worked. The risk team (Second Line) reviewed documentation and approved it. Internal audit (Third Line) wasn't involved—AI wasn't in their scope.

Two years later, regulators found the model had been discriminating against certain applicants the entire time. Each line had done their job as defined, but nobody had done comprehensive bias testing. The data scientists assumed risk would catch it. Risk assumed data scientists had tested for it. Audit never looked.

This is what happens when the Three Lines Model isn't properly applied to AI—when lines don't understand their AI responsibilities, when handoffs have gaps, or when one line is missing entirely.

The Three Lines Model, when applied correctly, creates defense in depth: multiple independent checks that catch what others miss. For AI governance, this layered approach is essential because AI risks are complex, evolving, and easy to miss.
