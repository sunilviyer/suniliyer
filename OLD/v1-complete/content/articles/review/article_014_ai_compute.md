# Article 14: AI Compute – Why GPUs Rule the AI World

## Introduction

Here's a number that should get your attention: training GPT-4 reportedly required around 25,000 NVIDIA A100 GPUs running for approximately 100 days. At current prices, that's roughly $100 million in compute alone—not counting the data, the researchers, or the infrastructure to connect it all.

AI doesn't run on magic. It runs on silicon—specifically, highly specialized chips that can perform trillions of mathematical operations per second. Understanding AI compute isn't just a technical curiosity; it's fundamental to understanding why AI development is concentrated among a few wealthy organizations, why geopolitics now involves semiconductor export controls, and why your organization's AI ambitions may be constrained by hardware you've never thought about.

For governance professionals, compute literacy matters. The hardware layer shapes what's possible, who can do it, and at what cost. This article explains how AI compute works, why GPUs dominate, who controls the supply chain, and what it all means for AI governance.
...
| Model | Parameters | Training Compute (FLOPs) |
|-------|------------|-------------------------|
| GPT-2 (2019) | 1.5 billion | ~10^20 |
| GPT-3 (2020) | 175 billion | ~10^23 |
| GPT-4 (2023) | ~1.8 trillion* | ~10^25 |
| Gemini Ultra | Unknown | Comparable to GPT-4 |
...
Traditional CPUs (Central Processing Units) are designed for general-purpose computing—running your operating system, executing programs, handling diverse tasks. They're optimized for:

- Complex logic and branching
- Sequential operations
- Low latency (fast response to individual tasks)
- Versatility across task types

**CPU architecture:**
- Few powerful cores (4-64 typically)
- Each core handles complex operations
- Optimized for single-threaded performance
- High clock speeds, sophisticated caching

**The problem for AI:**
AI needs to perform the same simple operation (multiply, add) billions of times in parallel. CPUs do complex things serially; AI needs simple things done massively in parallel.

### Enter the GPU

GPUs (Graphics Processing Units) were originally designed for rendering video game graphics—which, it turns out, requires exactly the kind of parallel computation AI needs.

**Rendering a game frame:**
- Calculate lighting for millions of pixels
- Apply the same calculations to each pixel
- Do it 60+ times per second
- Massively parallel, same operation repeated

**Training a neural network:**
- Multiply millions of weights by inputs
- Apply the same calculations to each weight
- Do it billions of times during training
- Massively parallel, same operation repeated

The similarity is not coincidental. Both tasks involve matrix multiplication—the mathematical operation at the heart of neural networks.

**GPU architecture:**
- Thousands of simpler cores (modern GPUs: 10,000+)
...
|----------------|-----|-----|
| Cores | 4-64 | 5,000-16,000+ |
| Core complexity | High | Low |
| Optimization | Latency (speed per task) | Throughput (tasks per second) |
| Best for | Diverse, sequential tasks | Parallel, repetitive tasks |
| AI training performance | Baseline | 10-100x faster |
