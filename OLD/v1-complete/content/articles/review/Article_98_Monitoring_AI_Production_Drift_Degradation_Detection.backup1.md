# Article 98: Monitoring AI in Production – Drift, Degradation, and Detection

## TL;DR

AI models don't stay good forever. Unlike traditional software that works the same way until someone changes the code, AI models can silently degrade as the world changes around them. Monitoring AI in production means watching for data drift (input data changing from what the model was trained on), model drift (predictions becoming less accurate), concept drift (the relationship between inputs and outputs changing), and performance degradation (the model simply getting worse). This article covers what to monitor, how to detect problems, when to alert, and what to do when monitoring reveals issues—because the alternative is discovering your AI has been wrong for months.

---

## Introduction

A fraud detection model was deployed with 95% accuracy. Six months later, it was still reporting 95% accuracy on its internal metrics. Everyone thought it was working fine.

But fraud losses were climbing. When someone finally investigated, they discovered the model was flagging transactions that looked like fraud from two years ago—but fraudsters had changed their tactics. The model was confidently wrong, and nobody noticed because nobody was watching the right things.

This is the silent failure mode of AI: the model keeps running, keeps making predictions, keeps reporting metrics—but the predictions no longer match reality.

Monitoring AI in production is how you detect these problems before they cause serious harm. It's different from monitoring traditional software because AI can fail in ways that look like success. The system is up, latency is fine, no errors in the logs—but the model is slowly becoming useless.

This article covers how to monitor AI systems effectively.
