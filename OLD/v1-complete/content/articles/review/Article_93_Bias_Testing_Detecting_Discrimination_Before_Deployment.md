# Article 93: Bias Testing – Detecting Discrimination Before Deployment

## TL;DR

Bias testing is the systematic process of checking whether an AI system treats different groups unfairly. It's not about finding bugs—it's about finding discrimination that may be hidden in patterns learned from historical data or introduced through design choices. Bias testing examines multiple fairness metrics (like demographic parity, equal opportunity, and predictive parity), recognizes that these metrics can conflict, and requires deciding which type of fairness matters most for your use case. This article covers why bias happens, how to test for it, which metrics to use, and what to do when you find it—because in AI governance, finding bias before deployment is infinitely better than discovering it after harm has occurred.

---

## Introduction

In 2018, Amazon scrapped an AI recruiting tool after discovering it systematically penalized resumes that contained the word "women's"—as in "women's chess club captain." The model had learned from a decade of hiring data where men dominated technical roles. It concluded that being associated with women was a negative signal.

Amazon's engineers didn't program this bias. They didn't intend it. They probably didn't even know about it until someone tested for it.

This is the challenge of AI bias: it's often invisible until you look for it. AI systems learn patterns from data—including patterns of historical discrimination that we might not want to perpetuate.

Bias testing is the process of looking for it. It's asking: "Does this AI treat different groups unfairly?" And doing so systematically, before the AI is deployed on real people.

This article covers how to conduct bias testing effectively.
