# Article 92: Testing AI Systems – Beyond Traditional Software QA

## TL;DR

Testing AI is fundamentally different from testing traditional software. With regular software, the same input always produces the same output—you can write a test that says "given input X, expect output Y." With AI, outputs are probabilistic, behavior emerges from data, and "correct" may be subjective or context-dependent. AI testing requires new approaches: testing data quality (not just code quality), testing model behavior (not just outputs), testing fairness (ensuring equitable treatment), testing robustness (handling unusual inputs), and testing in production (because AI degrades over time). This article covers the testing strategies that separate trustworthy AI from "we deployed it and hoped for the best."

---

## Introduction

A software engineer once told me: "I tested the model. I ran it on some examples, and it got them right. We're good to deploy."

A week later, customers were complaining. The model worked perfectly on examples similar to the test cases—but failed spectacularly on edge cases, unusual inputs, and populations not well represented in the test data.

That's not really testing. That's spot-checking.

Testing AI systems requires thinking differently about what "testing" means:

- **Traditional software:** Does the code do what it's supposed to?
- **AI systems:** Does the model behave appropriately across all the situations it will encounter?

The second question is much harder to answer. AI behavior emerges from data and training, not explicit programming. You can't just read the code to understand what it does. You have to probe, test, and verify—and you have to keep testing because AI systems can degrade over time.

This article covers how to test AI systems properly.
...
|--------|---------------------|------------|
| **Behavior source** | Explicitly programmed | Learned from data |
| **Determinism** | Same input → same output | May vary; probabilistic |
| **Correctness** | Defined by specification | Often subjective or contextual |
| **Test coverage** | Can enumerate paths | Input space is infinite |
| **Failure modes** | Usually obvious errors | Subtle degradation, edge cases |
| **Debugging** | Read code to understand | Model may be opaque |

### The Testing Challenge

In traditional software, you can achieve high test coverage—you might test every function, every branch, every path.

In AI, the input space is essentially infinite. A model that accepts images can receive an unlimited variety of images. A model that processes text can receive any combination of words. You can't test everything.

Testing AI is about strategic sampling: testing enough diverse cases to have confidence the model behaves appropriately, while knowing you can never test all cases.

---

## Layers of AI Testing

### Layer 1: Code Testing

AI systems still have code—data pipelines, preprocessing, model loading, inference, post-processing. This code needs traditional testing.
...
def test_feature_engineering():
    """Test that age bucketing works correctly."""
    input_data = {"age": 35}
    result = engineer_features(input_data)
    assert result["age_bucket"] == "30-40"
```

### Layer 2: Data Testing

AI behavior comes from data. Testing data is as important as testing code.

**Data quality tests:**
- No unexpected nulls or missing values
- Values within expected ranges
- Expected distributions
- No duplicates (or expected duplicates)
...
def test_training_data_quality():
    """Verify training data meets quality requirements."""
...
### Layer 3: Model Testing

Testing the trained model itself—does it perform as expected?

**Performance testing:**
- Meets accuracy/precision/recall thresholds
- Meets latency requirements
- Handles expected load

**Behavioral testing:**
- Expected behavior on known inputs
- Reasonable outputs for edge cases
- Consistent behavior (if determinism expected)
...
Testing the complete AI system in context.

**End-to-end testing:**
- Complete workflow from input to action
