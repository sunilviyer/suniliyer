...
export async function getAllArticles(): Promise<CompiledArticle[]> {
   ...
   *
   * Date parsing adds 'T00:00:00' to treat the date as midnight in local time.
   * Without this suffix, Date constructor interprets "YYYY-MM-DD" as UTC midnight,
   * which can cause timezone shifts that display dates incorrectly.
   *
   * Example without 'T00:00:00':
   *   "2025-12-12" → parsed as UTC midnight → displays as "December 11, 2025" in EST/PST
   *
   * Example with 'T00:00:00':
   *   "2025-12-12T00:00:00" → parsed as local midnight → displays as "December 12, 2025"
   *
   * This ensures that an article dated "2025-12-12" always displays as "December 12, 2025"
   * regardless of the user's timezone.
   *
   * Implementation: Descending sort where newer dates (larger timestamps) come first.
   */
  articles.sort((a, b) => {
