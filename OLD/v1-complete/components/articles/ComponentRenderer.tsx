...
export interface ComponentMetadata {
  type: 'table' | 'checklist' | 'flowchart' | 'template' | 'list' | 'image_prompt';
  label: string;
  section: string;
  id: string;
  template_link?: string;
  prompt?: string; // For image generation prompts
}
...
function ComponentSection({
  ...
  const renderImagePrompt = () => (
    <div className="image-prompt-component">
      <div className="prompt-content">
        <h4>Image Generation Prompt</h4>
        <div className="prompt-text">
          <pre><code>{metadata.prompt || content}</code></pre>
        </div>
      </div>
      <div className="prompt-actions">
        <button
          onClick={() => navigator.clipboard.writeText(metadata.prompt || content)}
          className="copy-prompt-btn"
        >
          Copy Prompt
        </button>
        <div className="prompt-info">
          <small>Use this prompt with DALL-E, Midjourney, or Stable Diffusion</small>
        </div>
      </div>
    </div>
  );
