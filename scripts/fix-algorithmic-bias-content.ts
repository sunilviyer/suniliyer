...
async function fixAlgorithmicBiasContent() {
  console.log('Fixing algorithmic-bias corrupted content...\n');

  // The HTML content for algorithmic-bias article
  // This is a cleaned version from the markdown file, converted to HTML
  const content = `
<h2 class="content-h2">
    ...
    SET yaml_content = jsonb_set(
      jsonb_set(
        jsonb_set(
          ...
          '{keyLearnings}',
          ${JSON.stringify(keyLearnings)}::jsonb
        ),
        '{additionalResources}',
        ${JSON.stringify(additionalResources)}::jsonb
      ),
      '{sources}',
